...
function setProperties( sp, fl ) {					// ***2.0***
  ...
  sp.rolloffFactor = 0.0; 	  //sp.maxDistance = 24;	sp.refDistance = 0; sp.rolloffFactor = 0.05  v3.0
...
function setDelay() {
  ...
  xs = pannerCL.positionX.value; ys = pannerCL.positionY.value; zs = -pannerCL.positionZ.value
	...
	delayRR.delayTime.value=dr;	//setValueAtTime(dr, audioCtx.currentTime);	
		//delayRR.delayTime.value=0.2/340;	console.log(df/340)
	delayBL.delayTime.value=dw;	//.setValueAtTime(dw, audioCtx.currentTime);	
		//delayRL.delayTime.value=0.2/340;
...
function defpos() {
 ...
 document.getElementById("zValue").innerHTML="pos_z = "+ zv;
  document.querySelector("#zv").value = zv;
		...
		document.getElementById("bassValue").innerHTML="bass = "+ bv;
   		  document.querySelector("#bass").value = bv;	
 setPos(xv,yv,zv); changeBass(); changeTreble(); 	changeZV(zv)	//== v3 ==	
 savefxyz()
}
...
function changeTreble() {
 ...
 tv = tvalue; 	tv=tv+ofst; tvH = tv+15;	//console.log( zv,tv,tvH)	// < 171 >
  if (fname) { 
...
function changeZV(z) {
  ...
  zv = z; 
    document.getElementById("zValue").innerHTML="pos_z = "+ zv;
 setPos( xv, yv, zv );	
	tv=tv+ofst; tvH = (tv+15)*10/zv;					// -- V3 --
	trebleLH.gain.value = tvH; trebleRH.gain.value = tvH;	console.log( zv,tvH)
}
//function changeSPv() {	
